...
// Transform API Credit Officer to UI format
const transformApiToCreditOfficer = (
    apiOfficer: ApiCreditOfficer,
): CreditOfficer => {
    // Determine status based on some logic (e.g., if they have active loans or recent activity)
    // For now, default to 'Active' as most officers would be active
    const status: "Active" | "Inactive" = "Active"; // You can add logic here later

    return {
        id: String(apiOfficer.id),
        name: `${apiOfficer.firstName} ${apiOfficer.lastName}`.trim(),
        idNumber: String(apiOfficer.id).slice(-5),
        status,
        phone: apiOfficer.mobileNumber || "N/A",
        email: apiOfficer.email,
        dateJoined: formatDate(apiOfficer.createdAt) || "N/A",
        branch: apiOfficer.branch,
        metrics: apiOfficer.metrics,
    };
};
...
export default function CreditOfficersPage() {
    ...
    const [selectedOfficers, setSelectedOfficers] = useState<string[]>([]);
    const [searchQuery, setSearchQuery] = useState("");
    const [sortColumn, setSortColumn] = useState<keyof CreditOfficer | null>(
        null,
    );
    const [sortDirection, setSortDirection] = useState<"asc" | "desc">("asc");
    const [currentPage, setCurrentPage] = useState(1);
    ...
    // API data state
    const [creditOfficersData, setCreditOfficersData] = useState<
        CreditOfficer[]
    >([]);
    const [totalCreditOfficers, setTotalCreditOfficers] = useState(0);
    const [creditOfficersStatistics, setCreditOfficersStatistics] = useState<
        StatSection[]
    >([]);
    ...
    const [selectedOfficer, setSelectedOfficer] =
        useState<CreditOfficer | null>(null);
    ...
    // Fetch credit officers data from HQ Manager API
    const fetchCreditOfficersData = useCallback(
        async (page: number = 1, search?: string) => {
            try {
                ...
                console.log(
                    "👔 Fetching credit officers using HQ Manager Service...",
                );

                // Get credit officers from HQ Manager API
                const response = await hqManagerService.getCreditOfficers({
                    page,
                    limit: itemsPerPage,
                    // API doesn't support search directly, we'll filter client-side
                });

                console.log("✅ Credit officers response:", response);

                // The response structure: { data, total, page, limit, totalPages }
                const apiOfficers = response.data as ApiCreditOfficer[];
                const total = response.total as number;

                setTotalCreditOfficers(total);

                // Transform API officers to UI format
                let transformedOfficers = apiOfficers.map(
                    transformApiToCreditOfficer,
                );

                // Apply client-side search if provided
                if (search && search.trim()) {
                    const query = search.toLowerCase();
                    transformedOfficers = transformedOfficers.filter(
                        (officer) =>
                            officer.name.toLowerCase().includes(query) ||
                            officer.email.toLowerCase().includes(query) ||
                            officer.phone.includes(query) ||
                ...
                if (selectedPeriod || dateRange) {
                    ...
                    if (selectedPeriod && selectedPeriod !== "custom") {
                        ...
                        transformedOfficers = filterByTimePeriod(
                            transformedOfficers.map((co) => ({
                                ...co,
                                createdAt: co.dateJoined,
                            })),
                            "createdAt",
                            selectedPeriod,
                        );
                        ...
                        // Apply custom date range filter
                        transformedOfficers = filterByDateRange(
                            transformedOfficers.map((co) => ({
                                ...co,
                                createdAt: co.dateJoined,
                            })),
                ...
                setCreditOfficersData(transformedOfficers);

                // Calculate statistics from the data
                const totalLoanValue = apiOfficers.reduce(
                    (sum, officer) => sum + officer.metrics.totalLoanValue,
                    0,
                );
                const totalLoans = apiOfficers.reduce(
                    (sum, officer) => sum + officer.metrics.totalLoans,
                    0,
                );
                const avgApprovalRate =
                    apiOfficers.length > 0
                        ? apiOfficers.reduce(
                              (sum, officer) =>
                                  sum + officer.metrics.approvalRate,
                              0,
                          ) / apiOfficers.length
                        : 0;
        ...
        - Avg Approval Rate: ${avgApprovalRate}%
      `);

                const statistics: StatSection[] = [
                    {
                        label: "Total Credit Officers",
                        value: total,
                        change: 0, // No historical data for change yet
                        changeLabel: "Active credit officers",
                        isCurrency: false,
                    },
                    {
                        label: "Total Loans",
                        value: totalLoans,
                        change: 0,
                        changeLabel: "Loans managed by officers",
                        isCurrency: false,
                    },
                    {
                        ...
                        change: 0,
                        changeLabel: "Total value of all loans",
                        isCurrency: true,
                    },
                    {
                        ...
                        change: 0,
                        changeLabel: "Average approval rate",
                        isCurrency: false,
                        suffix: "%",
                    },
                ];

                setCreditOfficersStatistics(statistics);
            } catch (err) {
                console.error("❌ Failed to fetch credit officers data:", err);
                setApiError(
                    err instanceof Error
                        ? err.message
                        : "Failed to load credit officers data",
                );
                error("Failed to load credit officers data. Please try again.");
            } finally {
                setIsLoading(false);
        ...
        [selectedPeriod, dateRange, itemsPerPage, error],
    );
    ...
    const debouncedSearch = useMemo(() => {
        ...
        return (searchTerm: string) => {
            clearTimeout(timeoutId);
            timeoutId = setTimeout(() => {
                fetchCreditOfficersData(1, searchTerm);
            }, 300);
        };
    }, [fetchCreditOfficersData]);
    ...
    useEffect(() => {
        if (hasInitiallyLoaded) {
            fetchCreditOfficersData(1, searchQuery);
        }
    }, [
        selectedPeriod,
        dateRange,
    ...
    useEffect(() => {
        if (hasInitiallyLoaded) {
            fetchCreditOfficersData(currentPage, searchQuery);
        }
    }, [currentPage, fetchCreditOfficersData, hasInitiallyLoaded]);
    ...
    const handleFilterClick = () => {
        // You can implement advanced filters here if needed
        console.log("Filter clicked");
    };
    ...
    const handleSave = async (updatedOfficer: CreditOfficer) => {
        try {
            ...
            success(
                `Credit Officer "${updatedOfficer.name}" updated successfully!`,
            );

            // TODO: Implement actual API call when update endpoint is available
            // For now, just log the update
            console.log("Update credit officer:", updatedOfficer);
        } catch (err) {
            console.error("Failed to update credit officer:", err);
    ...
    const confirmDelete = async () => {
        if (officerToDelete) {
            try {
                ...
                // Optimistic update - remove from UI immediately
                const updatedData = creditOfficersData.filter(
                    (officer) => officer.id !== officerToDelete.id,
                );
                setCreditOfficersData(updatedData);

                // Update statistics
                setTotalCreditOfficers((prev) => prev - 1);
                ...
                success(
                    `Credit Officer "${officerToDelete.name}" deleted successfully!`,
                );

                // TODO: Implement actual API call when delete endpoint is available
                // For now, just log the deletion
                console.log("Delete credit officer:", officerToDelete);
            } catch (err) {
                console.error("Failed to delete credit officer:", err);
    ...
    const totalPages = Math.ceil(totalCreditOfficers / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const paginatedOfficers = sortedOfficers.slice(startIndex, endIndex);
                ...
                <div className="max-w-[1150px]">
                    <div>
                        ...
                        <div className="mb-8">
                            ...
                            >
                                Manage credit officers and their performance
                            </p>
                            {/* Breadcrumb line */}
                            <div className="w-[18px] h-[2px] bg-black mt-2"></div>
                        </div>
                        ...
                        >
                            {isLoading ? (
                                ...
                                <StatisticsCard
                                    sections={creditOfficersStatistics}
                                />
                            )}
                        </div>
                        ...
                        <div className="max-w-[1041px] mb-6">
                            <div className="relative">
                                <input
                                    type="text"
                                    placeholder="Search credit officers by name, email, phone, or branch..."
                                    value={searchQuery}
                                    onChange={(e) =>
                                        ...
                                        {paginatedOfficers.length === 0 ? (
                                            <div className="p-12 text-center">
                                                ...
                                                <p className="text-gray-500">
                                                    {searchQuery
                                                        ? `No credit officers match your search "${searchQuery}".`
                                                        : "No credit officers are available in your branches."}
                                                </p>
                                                {searchQuery && (
                                                    <button
                                                        onClick={() =>
                                                    ...
                                                    <tr
                                                        style={{
                                                            backgroundColor:
                                                                "var(--bg-gray-50)",
                                                            borderBottom:
                                                                "1px solid var(--color-border-gray-200)",
                                                        }}
                                                    >
                                                        <th className="px-6 py-3 text-left">
                                                            <Checkbox
                                    ...
                                    {totalPages > 0 && (
                                        <div className="mt-4 flex items-center justify-between">
                                            <div className="text-sm text-gray-600">
                                                ...
                                                of {totalCreditOfficers} results
                                            </div>
                                            <Pagination
                                                totalPages={totalPages}
                                                page={currentPage}
