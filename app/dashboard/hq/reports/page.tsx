...
import ReportsFiltersModal, {
    ReportsFilters,
} from "@/app/_components/ui/ReportsFiltersModal";
import ReportDetailsModal, {
    ReportDetailsData,
} from "@/app/_components/ui/ReportDetailsModal";
import {
...
import { HQBranchReportButton } from "@/app/components/HQBranchReportButton";
...
export default function ReportsPage() {
    ...
    useEffect(() => {
        ...
        if (session && !allowedRoles.includes(session.role)) {
            ...
            const redirectPath =
                roleRedirects[session.role as keyof typeof roleRedirects] ||
                "/dashboard";
    ...
    const fetchReportsData = async (filters?: APIReportFilters) => {
        try {
            ...
            if (appliedFilters.reportType) {
                apiFilters.reportType =
                    appliedFilters.reportType.toLowerCase() as
                        | "daily"
                        | "weekly"
                        | "monthly";
            }
            ...
            const [reportsResponse, statisticsResponse] = await Promise.all([
                ...
                dashboardService
                    ...
                    .catch((statsError) => {
                        console.warn(
                            "Failed to fetch report statistics, using defaults:",
                            statsError,
                        );
            ...
            console.log(
                "📊 HQ Reports - Statistics response:",
                statisticsResponse,
            );

            // Safely handle reports response structure - PaginatedResponse has {data: [], pagination: {}}
            const rawReportsData = Array.isArray(reportsResponse?.data)
            ...
            console.log(
                "🔍 HQ Reports - First report sample:",
                rawReportsData[0],
            );
            ...
            const reportsData = rawReportsData.map((report: any) => ({
                ...
                creditOfficer: report.submittedBy
                ...
                dateSent:
                    ...
                    "",
                timeSent:
                    report.submittedAt?.split("T")[1]?.split(".")[0] || "",
                reportType: (report.type === "quarterly" ||
                report.type === "annual" ||
                report.type === "custom"
                    ? "monthly"
                    : report.type) as "daily" | "weekly" | "monthly",
                ...
                repaymentsCollected: parseFloat(
                    report.totalRecollections || "0",
                ),
            ...
            console.log(
                "🔄 HQ Reports - Transformed first report:",
                reportsData[0],
            );
            ...
            const totalCount =
                reportsResponse?.pagination?.total || reportsData.length || 0;
            const backendTotalPages =
                reportsResponse?.pagination?.totalPages || 1;
        ...
        try {
            ...
            const response = await reportsService.getBranchAggregateReports({
                ...
                reportType: appliedFilters.reportType
                    ? (appliedFilters.reportType.toLowerCase() as any)
                    : undefined,
                status: appliedFilters.reportStatus
                    ? (appliedFilters.reportStatus.toLowerCase() as any)
                    : undefined,
            });
    ...
    const handlePeriodChange = (period: TimePeriod) => {
        ...
        if (viewMode === "reports") fetchReportsData();
        else fetchBranchAggregates();
    };
    ...
    const handleReportClick = async (report: Report) => {
        try {
            ...
            const detailedReport = await reportsService.getReportById(
                report.id,
            );
    ...
    const handleApproveReport = async () => {
        if (selectedReportForDetails) {
            try {
                ...
                success(
                    `Report "${selectedReportForDetails.reportId}" approved successfully!`,
                );
    ...
    const handleDeclineReport = async () => {
        if (selectedReportForDetails) {
            try {
                ...
                success(
                    `Report "${selectedReportForDetails.reportId}" declined.`,
                );
                              ...
                              (viewMode === "branches" &&
                                ...
                                >
                                    <p
                                        ...
                                        style={{
                                            fontFamily:
                                                "'Open Sauce Sans', sans-serif",
                                        }}
                                    >
                                        {apiError
                                            ? "Failed to load data. Please try again."
                                            : `No ${viewMode === "branches" ? "branch data" : "reports"} found matching the selected filters.`}
                                    </p>
                                </div>
                            ) : (
                                <>
            ...
            {selectedReportForDetails && (
                <ReportDetailsModal
                    ...
                    reportData={{
                        ...
                        branch:
                            selectedReportForDetails.branch ||
                            "Report Overview",
                        ...
                        loansValueDispursed:
                            selectedReportForDetails.loansValueDispursed,
                        savingsCollected:
                            selectedReportForDetails.savingsCollected,
                        repaymentsCollected:
                            selectedReportForDetails.repaymentsCollected,
                        approvalHistory:
                            selectedReportForDetails.approvalHistory,
                    }}
