...
// import { enhancedUserService, EnhancedUser } from '@/lib/services/enhancedUserService';
...
// const transformUserToCustomer = (user: any): Customer => ({
//   id: user.id.toString(),
...
//       console.log('🎯 HQ Manager - Fetching customers using Enhanced User Service...');
...
//           filteredCustomers = filterByTimePeriod(filteredCustomers as any, 'createdAt', selectedPeriod) as unknown as EnhancedUser[];
//         } else if (dateRange) {
//           // Apply custom date range filter
//           filteredCustomers = filterByDateRange(filteredCustomers as any, 'createdAt', dateRange) as unknown as EnhancedUser[];
//         }
...
//       const transformedCustomers = (filteredCustomers as any[]).map(transformUserToCustomer);
...
import CustomersTable from "@/app/_components/ui/CustomersTable";
import EditCustomerModal from "@/app/_components/ui/EditCustomerModal";
import CustomersAdvancedFiltersModal, {
    CustomerAdvancedFilters,
} from "@/app/_components/ui/CustomersAdvancedFiltersModal";
import { ToastContainer } from "@/app/_components/ui/ToastContainer";
import { useToast } from "@/app/hooks/useToast";
import Pagination from "@/app/_components/ui/Pagination";
import {
...
// Transform API Customer to Customer format (matching CustomersTable interface)
const transformApiCustomerToCustomer = (
    ...
    phoneNumber: apiCustomer.mobileNumber,
    status:
        apiCustomer.verificationStatus === "verified" ? "Active" : "Scheduled",
    dateJoined: formatCustomerDate(apiCustomer.createdAt),
    branch: apiCustomer.branch,
    summary: apiCustomer.summary,
});

// Transform advanced filters to API params
const transformFiltersToApiParams = (filters: CustomerAdvancedFilters) => {
    const params: {
        branch?: string;
        search?: string;
    } = {};
    ...
    // Use search for name/email/phone filtering
    if (filters.search) {
        params.search = filters.search;
    }

    // Note: The API might not support all these filters directly
    // Additional client-side filtering may be needed for:
...
export default function CustomersPage() {
    ...
    const [advancedFilters, setAdvancedFilters] =
        useState<CustomerAdvancedFilters>({
            status: "",
            loanStatus: "",
            branch: "",
            creditOfficer: "",
    ...
    // API data state
    const [customers, setCustomers] = useState<Customer[]>([]);
    const [allCustomers, setAllCustomers] = useState<ApiCustomer[]>([]); // Store raw API data for client-side filtering
    const [customerStatistics, setCustomerStatistics] = useState<StatSection[]>(
        [],
    );
    const [isLoading, setIsLoading] = useState(true);
    ...
    // Fetch customers using the HQ Manager Service
    const fetchCustomersData = async (
        ...
        try {
            ...
            console.log(
                "🎯 HQ Manager - Fetching customers using HQ Manager Service...",
            );

            // Transform filters to API params
            const apiParams = transformFiltersToApiParams(
                filters || advancedFilters,
            );

            // Get customers from HQ Manager API
            const response = await hqManagerService.getCustomers({
                page,
                limit: itemsPerPage,
                ...apiParams,
            });

            console.log("✅ HQ Manager Service Response:", response);

            // The response structure from the API: { data, total, page, limit, totalPages }
            const apiCustomers = response.data as ApiCustomer[];

            console.log(
                `📊 Found ${apiCustomers.length} customers (total: ${response.total})`,
            );

            // Store raw API data
            setAllCustomers(apiCustomers);

            // Apply additional client-side filters that the API doesn't support
            let filteredCustomers = apiCustomers;

            // Apply verification status filter (maps to status in UI)
            if (filters?.status) {
                const verificationStatus =
                    filters.status === "Active" ? "verified" : "pending";
            ...
            // Apply loan status filter (client-side filtering based on summary)
            if (filters?.loanStatus) {
                filteredCustomers = filteredCustomers.filter((customer) => {
                    switch (filters.loanStatus) {
                        case "active":
                            return customer.summary.activeLoans > 0;
                        case "completed":
                            return (
                                customer.summary.totalLoans > 0 &&
                                customer.summary.activeLoans === 0
                            );
                        case "none":
                            return customer.summary.totalLoans === 0;
                        default:
                            return true;
                    }
                });
            ...
            // Apply loan amount range filters
            if (filters?.loanAmountMin) {
                const min = parseFloat(filters.loanAmountMin);
                if (!isNaN(min)) {
                    filteredCustomers = filteredCustomers.filter(
                        (customer) => customer.summary.totalLoans >= min,
                    );
                }
            }

            if (filters?.loanAmountMax) {
                const max = parseFloat(filters.loanAmountMax);
                if (!isNaN(max)) {
                    filteredCustomers = filteredCustomers.filter(
                        (customer) => customer.summary.totalLoans <= max,
                    );
                }
            }

            // Apply registration date range (client-side)
            ...
            // Apply time period and date range filters (client-side)
            if (selectedPeriod || dateRange) {
                const beforeDateFilter = filteredCustomers.length;
                const { filterByTimePeriod, filterByDateRange } =
                    await import("@/lib/utils/dateFilters");

                if (selectedPeriod && selectedPeriod !== "custom") {
                    // Apply preset time period filter
                    filteredCustomers = filterByTimePeriod(
                    ...
                    // Apply custom date range filter
                    filteredCustomers = filterByDateRange(
                        filteredCustomers as any,
                        "createdAt",
                        dateRange,
                    ) as unknown as ApiCustomer[];
                ...
                console.log(
                    `🔍 After date filter: ${filteredCustomers.length} (was ${beforeDateFilter})`,
                );
            }

            console.log(
                `🎯 Final filtered customers: ${filteredCustomers.length}`,
            );

            // Transform to customer format for display
            const transformedCustomers = filteredCustomers.map(
                transformApiCustomerToCustomer,
            );

            setCustomers(transformedCustomers);

            // For pagination, we need to recalculate based on filtered results
            // This is a workaround since the API pagination is based on unfiltered results
            setTotalCustomers(response.total); // Show total from API
            setTotalPages(response.totalPages); // Show total pages from API

            console.log(
                `🎯 Customers displayed: ${transformedCustomers.length}`,
            );
            ...
            const dashboardData = await hqManagerService.getDashboardSummary();
            ...
            const stats: StatSection[] = [
                {
                    ...
                    change: 0,
                    changeLabel: "Total customers in your branches",
                    isCurrency: false,
                },
                {
                    label: "Active Loans",
                    ...
                    change: 0,
                    changeLabel: "Total active loans across branches",
                    isCurrency: false,
                },
                {
                    label: "Total Savings",
                    value: dashboardData.summary.totalSavings,
                    change: 0,
                    changeLabel: "Total savings balance",
                    isCurrency: true,
                },
                {
                    label: "Branches",
                    value: dashboardData.summary.totalBranches,
                    change: 0,
    ...
    const handleAdvancedFiltersApply = async (
        filters: CustomerAdvancedFilters,
    ) => {
        setAdvancedFilters(filters);
        setCurrentPage(1); // Reset to first page when applying filters
    ...
    const handleSaveCustomer = async (updatedCustomer: Customer) => {
        try {
            ...
            // Use enhanced user service to get user with role information
            const userWithRole = await enhancedUserService.getUserById(
                updatedCustomer.id,
            );
            ...
            >
                <div className="max-w-[1150px]">
                    <div>
                        ...
                        {/* Customers Section Title */}
                        <div className="pl-4" style={{ marginBottom: "24px" }}>
                            <h2
                                className="font-semibold"
                                style={{
                                    fontSize: "18px",
                            ...
                            {isLoading ? (
                                ...
                                <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
                                    ...
                                    <div className="text-gray-600 text-sm">
                                        ...
                                        or there are no customers in the system.
                                    </div>
                                </div>
                            ) : (
                                <>
