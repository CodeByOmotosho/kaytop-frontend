...
export const API_CONFIG = {
    ...
    BASE_URL:
        process.env.NEXT_PUBLIC_KAYTOP_API_BASE_URL ||
        "https://kaytop-production.up.railway.app",
    ...
    LOG_API_CALLS: getEnvBoolean(
        "NEXT_PUBLIC_LOG_API_CALLS",
        process.env.NODE_ENV === "development",
    ),

    // Error tracking configuration
    ENABLE_ERROR_TRACKING: getEnvBoolean(
    ...
    RATE_LIMIT_MAX_REQUESTS: getEnvNumber(
        "NEXT_PUBLIC_RATE_LIMIT_MAX_REQUESTS",
        100,
    ),
    RATE_LIMIT_WINDOW_MS: getEnvNumber(
        "NEXT_PUBLIC_RATE_LIMIT_WINDOW_MS",
        60000,
    ), // 1 minute
    ...
    TOKEN_REFRESH_THRESHOLD: getEnvNumber(
        "NEXT_PUBLIC_TOKEN_REFRESH_THRESHOLD",
        300000,
    ), // 5 minutes before expiry
    AUTO_LOGOUT_ON_TOKEN_EXPIRE: getEnvBoolean(
        "NEXT_PUBLIC_AUTO_LOGOUT_ON_TOKEN_EXPIRE",
        true,
    ),
} as const;
...
export function validateConfiguration(): ConfigValidationResult {
    ...
    if (API_CONFIG.TIMEOUT > 120000) {
        warnings.push(
            "TIMEOUT is greater than 2 minutes, which may cause poor user experience",
        );
    }
    ...
    if (API_CONFIG.RETRY_ATTEMPTS > 10) {
        warnings.push(
            "RETRY_ATTEMPTS is greater than 10, which may cause long delays",
        );
    }

    if (API_CONFIG.RETRY_DELAY < 100) {
        warnings.push(
            "RETRY_DELAY is less than 100ms, which may cause server overload",
        );
    }
    ...
    if (!validLogLevels.includes(API_CONFIG.LOG_LEVEL)) {
        warnings.push(
            `LOG_LEVEL "${API_CONFIG.LOG_LEVEL}" is not valid. Valid values: ${validLogLevels.join(", ")}`,
        );
    }

    // Validate error tracking configuration
    if (
        API_CONFIG.ENABLE_ERROR_TRACKING &&
        !API_CONFIG.ERROR_TRACKING_ENDPOINT
    ) {
        warnings.push(
            "Error tracking is enabled but ERROR_TRACKING_ENDPOINT is not set",
        );
    }
 ...
 */
export function getConfigurationSummary(): Record<
    string,
...
export const API_ENDPOINTS = {
    ...
    ADMIN: {
        ...
        GUARANTOR_DETAILS: (id: string) =>
            `/admin/users/${id}/guarantor-details`,
        ID_VERIFICATION: (id: string) => `/admin/users/${id}/id-verification`,
        VERIFICATION_STATUS: (id: string) =>
            `/admin/users/${id}/verification-status`,
        UPDATE_USER: (id: string) => `/admin/users/${id}`,
        UPDATE_PROFILE_PICTURE: (id: string) =>
    ...
    LOANS: {
        ...
        LOAN_SUMMARY: (customerId: string) =>
            `/loans/customer/${customerId}/loan-summary`,
        DISBURSEMENT_SUMMARY: (customerId: string) =>
            `/loans/customer/${customerId}/disbursement-summary`,
    ...
    SAVINGS: {
        // Staff operations
        DEPOSIT: (customerId: string) =>
            `/savings/customer/${customerId}/deposit`,
        LOAN_COVERAGE: (customerId: string) =>
            `/savings/customer/${customerId}/loan-coverage`,
        APPROVE_LOAN_COVERAGE: (transactionId: string) =>
            `/savings/transactions/${transactionId}/approve-loan-coverage`,
        WITHDRAW: (customerId: string) =>
            `/savings/customer/${customerId}/withdraw`,
        APPROVE_WITHDRAWAL: (transactionId: string) =>
            `/savings/transactions/${transactionId}/approve-withdraw`,
        CUSTOMER_SAVINGS: (customerId: string) =>
            `/savings/customer/${customerId}`,
