...
function generateBarData(kpi?: OfficerDashboardResponse) {
    const months = [
        "Jan",
        "Feb",
        "Mar",
    ...
    // Use metrics from the CO dashboard response
    const baseValue = kpi?.metrics?.disbursedValue
        ? kpi.metrics.disbursedValue / 12 / 1000
        : 100;
    ...
    return months.map((name, index) => {
        ...
        return {
            ...
            repaid: Math.round(
                baseValue * 0.6 * monthVariation * seasonalFactor,
            ),
            savings: Math.round(
                baseValue * 0.4 * monthVariation * seasonalFactor,
            ),
        };
    });
}
...
function generateDonutData(kpi?: OfficerDashboardResponse) {
    return [
        { name: "Active loans", value: kpi?.metrics?.disbursedLoans ?? 60 },
        { name: "Missed payments", value: kpi?.metrics?.overdueLoans ?? 25 },
        { name: "New users", value: kpi?.recentReports?.length ?? 15 },
    ];
}
                ...
                <div className="px-3 pb-3">
                    <ul className="space-y-2">
                        <li className="flex items-center gap-2 text-sm text-muted-foreground">
                            <span className="w-3 h-3 rounded-full bg-[#5B21B6]" />{" "}
                            Active loans
                        </li>
                        <li className="flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        <li className="flex items-center gap-2 text-sm text-muted-foreground">
                            <span className="w-3 h-3 rounded-full bg-[#EDE9FE]" />{" "}
                            New users
                        </li>
                    </ul>
                </div>
