...
import { getOfficerMetricItems } from "@/lib/co-utils";
import { useBranchLoans } from "../../bm/queries/loan/useBranchLoans";
import { usePageChange } from "@/app/hooks/usePageChange";
import { DashboardKpiResponse, PaginationKey } from "@/app/types/dashboard";
import { useLoanDetails } from "../../bm/queries/loan/useLoanDetails";
import { CustomerHeader } from "@/app/_components/ui/CustomerHeader";
import { CustomerMetric } from "@/app/_components/ui/CustomerMetric";
import UserLoanTable from "@/app/_components/ui/UserLoanTable";
...
export default function LoanAgentClient() {
    ...
    // Use CO-specific dashboard hook for metrics
    const { isLoading, error, data } = useOfficerDashboard();

    // Keep using the existing BM loan fetching (it already filters by branch)
    const {
        isLoading: isLoadingLoan,
        error: branchLoanError,
        data: branchLoan,
    ...
    const headerData = data
        ? {
              branch: data.officerInfo?.branch || "", // Add branch property
    ...
    // Transform CO metrics to match CustomerMetric props
    const metricData = data ? getOfficerMetricItems(data) : [];
    ...
    return (
        <div className="space-y-6 px-6">
            {/* Header - now shows officer info if available */}
            <CustomerHeader
                title={""}
                data={headerData} // Use the transformed data
                isLoading={isLoading}
            />

            {/* Stats - now using CO-specific metrics */}
            <CustomerMetric
                item={metricData}
                cols={4}
                isLoading={isLoading}
                error={error as AxiosError<DashboardKpiResponse> | null} // Cast the error
            />

            {/* Table - still using the same loan fetching */}
            <div className="bg-white rounded-lg border overflow-hidden">
                <UserLoanTable
                    isLoading={isLoadingLoan}
                    error={branchLoanError}
            ...
            {selectedLoanId && (
                <div className="fixed inset-0 z-50 flex">
                    ...
                    <div className="w-full max-w-xl bg-white h-full shadow-xl px-6 overflow-y-auto">
                        ...
                        <h3 className="text-lg font-semibold mb-4 items-center text-center">
                            Loan Details
                        </h3>
                        ...
                        {isLoadingDetails ? (
                            ...
                            <>
                                ...
                                <LoanRepaymentTable
                                    ...
                                    item={loanDetails.data.repaymentHistory}
                                    customerDetails={
                                        loanDetails.data.customerDetails
                                    }
                                    loanDetails={loanDetails.data.loanDetails}
                                    meta={loanDetails.meta}
                                    onPageChange={(page) =>
                                        handlePageChange(
                                            page,
                                            PaginationKey.loan_page_repayment,
                                        )
                                    }
                                />
                            </>
