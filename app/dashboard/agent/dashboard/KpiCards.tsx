...
interface KpiCardsProps {
    data?: OfficerDashboardResponse; // Changed from DashboardKpi to OfficerDashboardResponse
    isLoading?: boolean;
}
...
export function KpiCards({ data, isLoading }: KpiCardsProps) {
    // Use CO dashboard metrics
    const todaysCollections = data?.metrics?.collectedValue ?? 0;
    const activeLoans = data?.metrics?.disbursedLoans ?? 0;
    const totalCustomers = data?.totalCustomers ?? 0; // You'll need to add this to your response

    const expectedCollections = data?.metrics?.disbursedValue ?? 0;
    const totalLoans = data?.metrics?.totalLoans ?? 0;
    const customerTarget = data?.metrics?.totalLoans ?? 0; // Or whatever target you want
    ...
    const customerProgress = getProgress(totalCustomers, customerTarget);
